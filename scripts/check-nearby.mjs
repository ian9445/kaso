...
import { distanceMeters, normalizeShops, getCurrentLocation, findNearbyShops } from "../assets/js/services/nearby.js";
...
test("query uses validated coordinates and does not send cookies", async () => {
  ...
  const fetcher = async (url, options) => {
    ...
    assert.match(options.body.get("data"), /around:1000,0,0/);
...
function mountHarness(t) {
  ...
  t.after(cleanup);
  return { nodes, cleanup, click: (id) => nodes[id].listeners.click() };
}

test("page starts without requesting location and ignores a callback after route cleanup", async (t) => {
